...
def save_player_status(user, status):
	user.status = status
	user.save()
...
def end_game_save_stats(game, players, win_player, tournament_id, room_state):
	user1 = MyUser.objects.get(username=players[0])
	user2 = MyUser.objects.get(username=players[1])
	win_user = MyUser.objects.get(username=win_player)
...
async def end_tournament_save_stats(winner, losers, tournament_name, tournament_id):
	...
	win_user = await sync_to_async(MyUser.objects.get)(username=winner)
	# winner_obj = await sync_to_async(Player.objects.get)(owner=win_user)
	stats.winner = win_user
	losers_users = await sync_to_async(MyUser.objects.filter)(username__in=losers)
	# losers_objs = await sync_to_async(Player.objects.filter)(owner__in=losers_users)
	losers_ids = await sync_to_async(lambda: [loser.id for loser in losers_users])()
...
class TournamentConsumer(AsyncWebsocketConsumer):
	...
	async def connect(self):
		...
		user_obj = await sync_to_async(MyUser.objects.get)(self.scope['user'])
		...
		if self.tournament_manager.create_or_join_room(tournament_name, username, nickname):
			await self.accept()
			await save_player_status(user_obj, "PLAYING")
		else:
			await self.close()
	...
	async def disconnect(self, close_code):
		...
		await save_player_status(user_obj, "ONLINE")
		if room:
			if room['state'] == TournamentStage["LOBBY"]:
				self.tournament_manager.remove_player(tournament_name, username)
				try:
					tournament_room = await sync_to_async(TournamentRoom.objects.get)(name=tournament_name)
					await sync_to_async(tournament_room.players.remove)(user_obj)
				except:
					pass
				await self.send_players_update()
			else:
				idx = self.tournament_manager.get_player_index(tournament_name, username)
				if idx > -1:
					...
					print(f'Player {username} not found in room')
