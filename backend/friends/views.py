	...
	def get(self, request):
		...
		if param == "initiated" :
			...
			for o in initiated_accepted :
				# user_received = MyUser.objects.filter(o.user_received.id).first()
				result.append({"username" : o.user_received.username, "status" : o.user_received.status, "avatar" : o.user_received.avatar.url})
			...
			for o in received_accepted :
				# player_initiated = MyUser.objects.filter(o.user_initiated.id).first()
				result.append({"username" : o.user_initiated.username, "status" : o.user_received.status, "avatar" : o.user_received.avatar.url})
			return Response(result, status=status.HTTP_200_OK)
...
class CreateFriendAPIView(APIView):
	...
	def post(self, request):
		try :
			user_received = MyUser.objects.get(username=request.data['username'])
...
class AcceptFriendAPIView(APIView) :
	...
	def patch(self, request):
		try :
			user_initiator = MyUser.objects.get(username=self.request.data["username"])
		except MyUser.DoesNotExist:
...
class RefuseFriendAPIView(APIView):
	...
	def patch(self, request):
		try :
			user_initator = MyUser.objects.get(username=self.request.data["username"])
		except MyUser.DoesNotExist:
			return Response("Ce user n'existe pas.", status=status.HTTP_400_BAD_REQUEST)
...
class DeleteFriendAPIView(APIView):
	...
	def delete(self, request) :
		try :
			former_friend = MyUser.objects.get(username=self.request.data["username"])
		except MyUser.DoesNotExist:
			return Response("Ce user n'existe pas.", status=status.HTTP_400_BAD_REQUEST)
