...
class RoomConsumerFour(AsyncWebsocketConsumer):
		...
		await self.channel_layer.group_send(self.room_name,
			{
				...
				'player_name': self.game_state_obj.players[self.position].user.nickname
			})
	...
	async def handle_new_connection(self, user):
		...
		player = await sync_to_async(MyUser.objects.get)(user)
	...
	async def start_game(self):
		...
		await sync_to_async(self.game_state_obj.game_history.add_player)(self.game_state_obj.players['player_left'].user)
		await sync_to_async(self.game_state_obj.game_history.add_player)(self.game_state_obj.players['player_right'].user)
		await sync_to_async(self.game_state_obj.game_history.add_player)(self.game_state_obj.players['player_top'].user)
		await sync_to_async(self.game_state_obj.game_history.add_player)(self.game_state_obj.players['player_bottom'].user)
	...
	async def ready(self, event):
		...
		await self.send(text_data=json.dumps({
			'type': event['type'],
			'player_left': self.game_state_obj.players['player_left'].user.nickname,
			'player_right': self.game_state_obj.players['player_right'].user.nickname,
			'player_top': self.game_state_obj.players['player_top'].user.nickname,
			'player_bottom': self.game_state_obj.players['player_bottom'].user.nickname
		}))
