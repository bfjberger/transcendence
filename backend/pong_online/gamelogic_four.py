...
class GameStateFour:
	...
	Attributes:
		...
		players (dict): A dictionary with the player positions as keys and MyUser objects as values.
	...
	Methods:
		add_player_to_dict(player_pos, user): Add a player to the game state dictionary.
	...
	def add_player_to_dict(self, player_pos, user):
		"""
		...
		Args:
			...
			user (MyUser model instance): instance of the model MyUser associated with the player
		"""
		self.players[player_pos] = self.Player(player_pos, user)
	...
	async def record_game_result(self, winner_pos, losers_pos):
		...
		if winner_pos is not None and loser_pos is not None:
			...
			if winner != "Not enough players":
				scores = {
					self.players["player_left"].user.username: self.players["player_left"].score,
					self.players["player_right"].user.username: self.players["player_right"].score,
					self.players["player_top"].user.username: self.players["player_top"].score,
					self.players["player_bottom"].user.username: self.players["player_bottom"].score
				}
